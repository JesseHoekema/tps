...
auth.onAuthStateChanged(user => {
    if (user) {
        ...
        updateCreditCardInfo(); // Voer deze functie uit om de creditcardinformatie bij te werken
        getNotes()
        checkDivVisibilityOnLoad();
    } else {
        // User is signed out
        localStorage.clear();
...
function register() {
    ...
    auth.createUserWithEmailAndPassword(email, password)
        .then(userCredential => {
            ...
            // Reset het registratieformulier
            document.getElementById('register-container').reset();
            // Verberg het registratieformulier en toon het hoofdcontainer
...
function getNotes() {
    console.log('getNotes is called'); // Voeg deze regel toe
    const user = auth.currentUser;
...
// test
// Voeg deze functie toe aan je app.js-bestand
async function displayVisibilityStatusOnLoad() {
    const user = auth.currentUser;

    try {
        // Haal de huidige zichtbaarheidsstatus op uit Firestore
        const doc = await firestore.collection('users').doc(user.uid).get();
        const userData = doc.data();
        const divVisible = userData?.divVisible || false;

        // Toon de zichtbaarheidsstatus in de console
        console.log('Zichtbaarheidsstatus bij het laden van de pagina:', divVisible);

        // Toggle de zichtbaarheid van het div-element op basis van de opgehaalde status
        const divContainer = document.getElementById('nav-bar');
        if (divVisible) {
        ...
        console.error('Fout bij het ophalen van de zichtbaarheidsstatus:', error.message);
    }
}

// Roep displayVisibilityStatusOnLoad aan wanneer de DOM is geladen
document.addEventListener('DOMContentLoaded', async () => {
    await displayVisibilityStatusOnLoad();
});

// Voeg deze functie toe aan je app.js-bestand
async function toggleDivVisibility() {
    const user = auth.currentUser;
    ...
    try {
        ...
        // Toggle de zichtbaarheid van het div-element
        const divContainer = document.getElementById('custom-div-container');
        if (divVisible) {
            divContainer.style.display = 'none';
        } else {
            divContainer.style.display = 'block';
        ...
        // Update de zichtbaarheidsstatus in Firestore
        await firestore.collection('users').doc(user.uid).update({
            divVisible: !divVisible,
        });

        // Toon de bijgewerkte zichtbaarheidsstatus in de console
        console.log('Bijgewerkte zichtbaarheidsstatus:', !divVisible);
    } catch (error) {
        console.error('Fout bij het bijwerken van de zichtbaarheidsstatus:', error.message);
    }
}
// other test
document.addEventListener('DOMContentLoaded', function () {
    checkDivVisibilityOnLoad(); // Roep deze functie aan wanneer de pagina is geladen
});

function checkDivVisibilityOnLoad() {
    const user = auth.currentUser;
    const divContainer = document.getElementById('custom-div-container');
    ...
    firestore.collection('users').doc(user.uid).get()
        .then(doc => {
            ...
            // Toggle de zichtbaarheid van het div-element op basis van de opgehaalde waarde
            if (divVisible) {
                divContainer.style.display = 'block';
            } else {
                divContainer.style.display = 'none';
            ...
            // Toon de zichtbaarheidsstatus in de console
            console.log('Visibility status on load:', divVisible);
        })
        .catch(error => console.error('Error getting user data:', error.message));
}
